...
feature 'user interacts with the shopping cart' do
  scenario 'user adds an event to the shopping cart and then removes it from the cart' do
    ...
    expect(page).to have_content event.balcony_ticket_price.to_s
    expect(page).to have_content event.floor_ticket_price.to_s
    expect(page).to have_xpath("//span[@class='badge' and contains(., '1')]")
    
    find("a[href='/cart_items/1']").click
    expect(page).to have_no_content event.name
    expect(page).to have_content "There are currently no items in the shopping cart."
  end
